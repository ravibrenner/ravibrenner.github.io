...
plot_similarities <- function(target_title, n = 5) {
  ...
  df |>
    ...
    labs(
      ...
      color = "Book title",
      title = str_wrap(str_c("Top ", n, " most similar books to ", target_title),45)
    ) +
    theme_minimal()
}
...
umap_res$layout |>
  ...
  theme_minimal() +
  labs(title = "Visual of all books", x = "Dimension 1", y = "Dimension 2")
```
...
Embeddings are also used for things like images. Image recognition networks sometimes develop [specific layers](https://youtu.be/UZDiGooFs54?si=NptXi-ueN_6iX2yM) that can recognize things like faces, animals, or shapes. But sometimes we can't understand what the layer is doing. If there is some discernible meaning, it's probably because there's such a strong relationship in the data, not because the model actually "understands" anything.
