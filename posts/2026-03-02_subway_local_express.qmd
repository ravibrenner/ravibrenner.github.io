...
express_stations_full <- temp |>
  mutate(removal_candidate = 
           if_else(parent_station_dest %in% temp$parent_station_orig,0,1),
         mismatch_route_num = if_else(num_routes_dest != num_routes_orig,1,0),
         remove = if_else(removal_candidate == 1 & mismatch_route_num == 1,1,0)) |>
  filter(remove == 0) |>
  select(stop_name_orig, stop_name_dest,routes_intersect)  |>
...
express_travel_times <- travel_times |>
  filter(stop_id %in% express_stations_full$stop_id) |>
  inner_join(mta$stops) |>
